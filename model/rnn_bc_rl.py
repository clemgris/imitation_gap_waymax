...
class make_train:
    ...
    def train(self,):
        ...
        init_rnn_train = ScannedRNN.initialize_carry((self.config["num_envs"], feature_extractor_shape))
        ...
        def _log_step(cary, unused):
            ...
            if self.obs_mask is not None:
                masked_obs = self.obs_mask.mask_obs(current_state, obs, rng)
            else:
                masked_obs = obs
        ...
        def _update_epoch(train_state, rng):
            ...
            def _update_scenario(train_state, scenario, rng):         
                ...
                obs = datatypes.sdc_observation_from_state(current_state_rl,
                                                            roadgraph_top_k=self.config['roadgraph_top_k'])
                if self.obs_mask is not None:
                    masked_obs = self.obs_mask.mask_obs(current_state_rl, obs, rng)
                else:
                    masked_obs = obs
                ...
                def _update(train_state, log_traj_batch, traj_batch_bc, traj_batch_rl, advantages, targets):
                    ...
                    def _loss_fn(params, init_rnn_state, log_traj_batch, traj_batch_bc, traj_batch_rl, advantages, targets):
                        
                        def _bc_loss_fn(params, init_rnn_state, log_traj_batch, traj_batch): # CONTINUOUS ACTION SPACE
                        ...
                        bc_loss = _bc_loss_fn(params, init_rnn_state, log_traj_batch, traj_batch_bc)
                    ...
                    total_loss, grads = grad_fn(train_state.params, init_rnn_state_train, log_traj_batch, traj_batch_bc, traj_batch_rl, advantages, targets)
                    train_state = train_state.apply_gradients(grads=grads)
                    return train_state, total_loss

                train_state, total_loss = _update(train_state, log_traj_batch, traj_batch_bc, traj_batch_rl, advantages, targets)
            ...
            #     data = self.data
                ...
                if not jnp.any(scenario.object_metadata.is_sdc):
                    ...
                    train_state, (loss, (bc_loss, rl_loss, value_loss, loss_actor, entropy)) = jit_update_scenario(train_state, scenario, rng)
                    rng = jax.random.split(random.PRNGKey(rng), num=1)[0, 0]
                    losses.append(loss)
